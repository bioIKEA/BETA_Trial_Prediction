...
def readTest(test_file='', drug_idx_file='', target_idx_file=''):
    ...
    with open(test_file) as f:
        ...
        for line in f:
            ...
            if (items[2] == 'true'):
                ...
                test_unknown_triples_set.add((items[0], items[1]))

    print(len(test_positive_triples_set) )
    print(len(test_negative_triples_set) )
    print(len(test_unknown_triples_set) )

    data_set = np.zeros((len(test_positive_triples_set) + len(test_negative_triples_set)+len(test_unknown_triples_set), 3), dtype=int)
    print('data_set: ',data_set.shape)
    count = 0
    count_1=0
    for pair in test_positive_triples_set:
        ...
        if (pair[0] in global_drugs and pair[1] in global_targets):
            ...
            print("not existed: %s\t %s\t %s\n" % (count_1, pair[0], pair[1]))
            count_1+=1
    ...
    for pair in test_unknown_triples_set:
